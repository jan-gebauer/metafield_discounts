/*
  ...
  - The required column `id` was added to the `MetafieldValue` table with a prisma-level default value. This is not possible if the table is not empty. Please add this column as optional, then populate it before making it required.
...
CREATE TABLE "new_Metafield" (
    ...
    CONSTRAINT "Metafield_metafield_value_id_fkey" FOREIGN KEY ("metafield_value_id") REFERENCES "MetafieldValue" ("id") ON DELETE RESTRICT ON UPDATE CASCADE
);
