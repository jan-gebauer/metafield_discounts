...
export const action = async ({ request }: ActionFunctionArgs) => {
    ...
    const response = await getProductsUpdateAfter({ admin, query: minimumDate.toString(), nextCursorParam: null })
    ...
    //     const response = await getProductsUpdateAfter({ admin, query: minimumDate.toString(), nextCursorParam: responseJson.data.products.pageInfo.endCursor });
    ...
    const itemsWithMetafields = items.filter((item: any) => item.node.metafields.edges.length > 0)
    console.log(itemsWithMetafields.length)
    const processedItems: { item: Item, metafields: Metafield[] }[] = itemsWithMetafields.map((edge: any) => {
    ...
    processedItems.forEach(async (processedItem: { item: Item, metafields: Metafield[] }) => {
                    ...
                    <Card>
                        <BlockStack gap="300">
                            <Form onSubmit={() => {
                                ...
                                <FormLayout>
                                    <TextField label="Starting date for sync" value={minimumDate} onChange={setMinimumDate} autoComplete="off" type="date" />
